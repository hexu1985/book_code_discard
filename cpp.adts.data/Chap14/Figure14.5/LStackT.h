/*-- LStack.h --------------------------------------------------------------
  ...
  Basic operations:
    ...
    repeatedpush: Adds several values at the top of a stack
...
class Stack
{
 ...
 /***** Assignment *****/
  ...
  // Declare push to be virtual for this to work correctly in Figure 14.5 
  virtual void push(const StackElement & value);
  //-- Incorrect 1st version:  void push(const StackElement & value);
  /*------------------------------------------------------------------------
  ...
  void repeatedPush(const StackElement & value, int number);  
  /*------------------------------------------------------------------------
    Add several copies of value to the stack.

    Precondition:  None.
    Postcondition: number copies of value are added at top of stack.     
  ------------------------------------------------------------------------*/ 
...
template <typename StackElement>
inline void Stack<StackElement>::repeatedPush(
                                 const StackElement & value, int number)
{
